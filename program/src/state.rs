...
pub struct SwapV1 {
    ...
    /// Add these fields to the SwapInfo struct
...
impl Pack for SwapV1 {
    ...
    fn pack_into_slice(&self, output: &mut [u8]) {
        ...
        let (
            ...
            admin_pubkey,
            paused,
        ) = mut_array_refs![output, 1, 1, 32, 32, 32, 32, 32, 32, 32, 64, 33, 32, 1];
        ...
        admin_pubkey.copy_from_slice(self.admin_pubkey.as_ref());
    ...
    fn unpack_from_slice(input: &[u8]) -> Result<Self, ProgramError> {
        ...
        let (
            ...
            admin_pubkey,
            paused,
        ) = array_refs![input, 1, 1, 32, 32, 32, 32, 32, 32, 32, 64, 33, 32, 1];
        Ok(Self {
...
mod tests {
    ...
    const TEST_ADMIN_PUBKEY: Pubkey = Pubkey::new_from_array([8u8; 32]);
    const TEST_PAUSED: bool = true;
    ...
    fn swap_v1_pack() {
        ...
        packed.extend_from_slice(&TEST_ADMIN_PUBKEY.to_bytes());
        packed.push(TEST_PAUSED as u8);
        let unpacked = SwapV1::unpack(&packed).unwrap();
        assert_eq!(swap_info, unpacked);
