...
pub enum SwapInstruction {
    ...
    /// Pause the token swap
    Pause,

    /// Unpause the token swap
    Unpause,

    /// Change the price of the token swap
...
impl SwapInstruction {
    ...
    pub fn unpack(input: &[u8]) -> Result<Self, ProgramError> {
        ...
        Ok(match tag {
            ...
            7 => Self::Unpause,
            8 => {
                let (new_price, _rest) = Self::unpack_u64(rest)?;
                Self::ChangePrice { new_price }
            }
    ...
    pub fn pack(&self) -> Vec<u8> {
        ...
        match self {
            ...
            Self::Pause => buf.push(6),
            Self::Unpause => buf.push(7),
            Self::ChangePrice { new_price } => {
                buf.push(8);
